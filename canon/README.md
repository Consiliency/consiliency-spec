...
| `$int` payload parsers disagree: Python `int()` takes `" 42 "`, `"1_000"`, Bengali digits; JS `BigInt()` takes `""`, `"0x1A"`; Rust `parse` takes `"+5"` | one grammar `^-?[0-9]+$` checked **before** the native parser in all three ports; vectors `reject-int-payload-*`, `int-payload-leading-zeros`, `int-payload-negative-zero` |
| JS `100.0 === 100`: a whole-valued float is indistinguishable from an integer | the public TS API takes **`bigint` only**; a plain `number` is rejected outright (not routed by integrality) |
